...
targetPackages <- c("ggplot2", "scales", "ggsignif") 
...
#pvalueの大きさに応じてアノテーションを付与する関数を定義
psymbol <- function (pvalue) 
{
        if (is.na(pvalue)) {
                return("NA")
        }
                ...
                return("pvalue is not in 0 ~ 1")
        }
}

getwd() #ワーキングディレクトリ確認
# データの読み込み
raw_dt <- read.csv("boxplotSigBar_input.csv", row.names = 1, header = T, stringsAsFactors = F)
dt <- raw_dt[1:(nrow(raw_dt) - 1), ]
dt$group <- factor(dt$group, levels = unique(dt$group)[unique(dt$order)]) #順序の指定

pvalue <- raw_dt[nrow(raw_dt), 1:(ncol(raw_dt) - 3)]
psymbol <- sapply(pvalue, FUN = psymbol)
...
sigSize <- 6 #有意差表示の大きさ
barSize <- 0.5 #有意差バーの厚さ←うまく繁栄されない
#N.S.とアスタリスクが同じ大きさだと*が小さく見えるので調整
sigSizeVec <- sapply(psymbol, function(i) {
        if(i == "N.S.") {
                return(sigSize * 0.7)
        } else {
                return(sigSize)
...
for(i in 1:variable_len) {
        ...
        sigdf <- data.frame(x = 1, xend = 2, y = max(dt[, i]) * 1.1, annotation = psymbol[i])
        g <- g + geom_signif(stat = "identity", 
                    data = sigdf, 
                    aes(x = x, xend = xend, y = y, yend = y, annotation = annotation, 
                        textsize = sigSizeVec[i]), 
                    size = barSize, inherit.aes = F)
        print(g)
        dev.off()
}
